...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	switch function {
	case "querySatellite":
		return s.querySatellite(APIstub, args)
	case "initLedger":
...
func (s *SmartContract) querySatellite(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments. Expecting 1")
	}

	satelliteAsBytes, _ := APIstub.GetState(args[0])
	return shim.Success(satelliteAsBytes)
}
